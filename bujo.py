...
# --- 4. STYLE & DESIGN IPAD (Correction Menus & Calligraphie) ---
...
<style>
    ...
    .stTabs [data-baseweb="tab-list"] {{ 
        display: flex; justify-content: center !important; gap: 30px; margin-bottom: 50px !important; 
    }}
    .stTabs button {{ font-family: 'Great Vibes', cursive !important; font-size: 2.2rem !important; color: var(--sapin) !important; }}

    .titre-calli {{ font-family: 'Great Vibes' !important; font-size: 4.8rem; color: var(--sapin); text-align: center; margin-bottom: 30px; }}
    .sous-titre-calli {{ font-family: 'Great Vibes' !important; font-size: 3rem; margin-top: 20px; }}
    
    /* POST-ITS */
    .post-it {{ background: white; border-radius: 15px; padding: 15px; margin-bottom: 10px; border-left: 8px solid var(--rose); box-shadow: 2px 2px 8px rgba(0,0,0,0.05); font-size: 1.3rem; }}
    .postit-planning {{ background: var(--postit-jaune); padding: 10px; border-left: 4px solid #FBC02D; margin-top: 5px; border-radius: 4px; }}
    .postit-menu {{ background: var(--postit-vert); padding: 8px; border-left: 4px solid #80CBC4; margin-top: 5px; border-radius: 4px; }}
    .postit-course {{ background: #E1F5FE; padding: 10px; border-left: 5px solid #03A9F4; border-radius: 8px; color: black !important; }}

    /* CALENDRIER */
    .cal-header {{ background: var(--vert-doux) !important; color: var(--sapin) !important; text-align: center; border-radius: 10px 10px 0 0; padding: 8px; font-weight: bold; }}
</style>
...
with tabs[1]:
    ...
    st.markdown(f'<div style="text-align:center; font-size:2.8rem; padding:15px;">Semaine du {start.strftime("%d/%m")}</div>', unsafe_allow_html=True)
...
# --- TAB 3: ANNEE (Calendrier Grille Rétabli) ---
with tabs[2]:
    ...
    for r in range(4):
        ...
        for c in range(3):
            m_idx = r * 3 + c + 1
            with cols_a[c]:
                st.markdown(f'<div class="cal-header">{calendar.month_name[m_idx].upper()}</div>', unsafe_allow_html=True)
                ...
                for w in cal:
                    ...
                    res += line + "\n"
                st.markdown(f'<pre style="background:white; padding:10px; border:1px solid var(--vert-doux); text-align:center; font-size:0.85rem; color:black;">{res}</pre>', unsafe_allow_html=True)

# --- TAB 4: TRACKERS (Lecture Détaillée + Missions) ---
with tabs[3]:
    ...
    with tr_tabs[0]:
        ...
        l_g = st.selectbox("Genre", ["Roman", "Bien-être", "Cuisine", "Thriller"], key="lg")
        l_cit = st.text_area("Citation ou avis", key="lcit")
        if st.button("💾 Sauver Livre", key="btn_l"):
            save_gs("Lecture", [datetime.now().strftime("%d/%m/%Y"), l_t, l_a, l_g, 5, l_cit]); st.rerun()
        st.markdown("---")
        ...
        for i, b in enumerate(reversed(l_data)):
            ...
            with st.expander(f"📔 {b.get('Titre')} - {b.get('Auteur')}"):
                st.write(f"Genre: {b.get('Genre')} | Citation: {b.get('Citation', b.get('Note'))}")
                if st.button("🗑️", key=f"dl_{idx_l}"): delete_gs("Lecture", idx_l); st.rerun()
    ...
    with tr_tabs[2]:
        ...
        if st.button("🚀 Valider Mission", key="btn_tr"): save_gs("Menage", [m_j, m_t, m_q]); st.rerun()

# --- TAB 5: COURSES (Favoris + Stickers) ---
with tabs[4]:
