...
# ==========================================
# 2. DESIGN & STYLE (Correction Visibilité)
# ==========================================
...
<style>
    ...
    /* --- CORRECTION VISIBILITÉ DES CHAMPS (INPUTS) --- */
    /* On force le fond en blanc et le texte en noir pour les entrées */
    ...
    /* Couleur du texte écrit par l'utilisateur */
    input, textarea, div[data-baseweb="select"] div {
        color: #000000 !important; 
        font-weight: 500 !important;
    }

    /* Labels (Titres au-dessus des cases) */
    .stMarkdown p, label {
...
if page == "📅 Daily Log":
    ...
    with col1:
        txt = st.text_input("Nouvelle pensée...", key="in_note", placeholder="Tape ici ce que tu as en tête...")
    with col2:
    ...
    note_libre = st.text_area("Écris ici tes pensées...", label_visibility="collapsed")
...
# --- PAGE FINANCES ---
    ...
    with st.expander("➕ Ajouter une opération", expanded=True):
        ...
        label = st.text_input("Libellé (ex: Loyer, Salaire...)")
    ...
    if data:
        ...
        if not df_mois.empty:
            st.write("---")
            st.write("### 📜 Historique")
            st.info("💡 Tu peux modifier les cases ci-dessous directement.")
            ...
            if col_btn1.button("💾 Enregistrer les modifications"):
            ...
            if col_btn2.button("🗑️ Supprimer les lignes cochées"):
            ...
            if st.button("📥 Télécharger le rapport PDF"):
                ...
                st.download_button("Confirmer le téléchargement", data=pdf_output, file_name=f"Budget_{sel_mois}.pdf")
        else:
