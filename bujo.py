...
<style>
    ...
    /* Forçage Inputs & Menus (Anti-Noir iPad) */
    ...
    .stButton>button { background-color: var(--rose) !important; color: white !important; border-radius: 15px !important; }
...
# --- TAB 4: TRACKERS (RESTORED VERSION NICKEL) ---
with tabs[3]:
    ...
    with tr_tabs[0]:
        ...
        l_g = st.selectbox("Genre", ["Roman", "Bien-être", "Cuisine", "Thriller", "BD", "Autre"], key="lg")
        l_n = st.select_slider("Ma Note ⭐", options=[1,2,3,4,5], value=5)
        l_av = st.text_area("Mes impressions / Citations", key="lav")
        if st.button("💾 Enregistrer dans la bibliothèque"):
            if l_t:
                save_gs("Lecture", [datetime.now().strftime("%d/%m/%Y"), l_t, l_a, l_g, l_n, l_av])
                st.rerun()
        
        st.markdown('<div class="sous-titre-calli">Ma Bibliothèque</div>', unsafe_allow_html=True)
        lib_data = load_gs("Lecture")
        for i, b in enumerate(reversed(lib_data)):
            idx_lib = len(lib_data)-1-i
            with st.expander(f"📔 {b.get('Titre')} - {b.get('Auteur')}"):
                ...
                if st.button("🗑️ Supprimer", key=f"del_lib_{idx_lib}"):
                    delete_gs("Lecture", idx_lib)
                    st.rerun()

    # 🩺 BIEN-ÊTRE RESTAURÉ
    with tr_tabs[1]:
        st.markdown('<div class="sous-titre-calli">Bilan Santé & Bien-être</div>', unsafe_allow_html=True)
        eau = st.slider("Consommation d'eau 💧 (verres)", 0, 12, 6)
        hum = st.select_slider("Mon Humeur du jour", options=["☀️", "🌤️", "☁️", "🌧️", "⛈️"])
        if st.button("Sauver le bilan"):
            save_gs("Sante", [datetime.now().strftime("%d/%m/%Y"), "", eau, hum])
    ...
    with tr_tabs[2]:
        ...
        actions_liste = ["Vaisselle", "Plier le linge", "Étendre le linge", "Rangement chambre", "Nettoyage SdB/WC", "Nettoyage sol", "Poubelles", "Cuisine"]
...
with tabs[4]:
    ...
    favs = ["🍞 Pain", "🥛 Lait", "🥚 Oeufs", "🍎 Fruits", "🍝 Pâtes", "🥣 Céréales", "🍚 Riz"]
